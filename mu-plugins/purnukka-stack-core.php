<?php
/**
 * Plugin Name: Purnukka Stack - Core Branding (v0.28)
 * Description: Master SPoT: Advanced Country Sync, 4-Color System, SMTP & Legal Restoration.
...
/**
 * MASTER DEEP SYNC WITH COUNTRY MAPPING
 */
function purnukka_sync_master_data() {
    ...
    // 2. PDF Sync with Country Code Conversion
    $pdf = get_option('wpo_wcpdf_settings_general', []);
    $c_country_name = get_option('p_legal_country', 'Finland');
    
    // Convert name to code for PDF plugin
    $codes = ['Finland' => 'FI', 'Sweden' => 'SE', 'Estonia' => 'EE', 'Norway' => 'NO', 'UK' => 'GB', 'USA' => 'US'];
    $c_country_code = isset($codes[$c_country_name]) ? $codes[$c_country_name] : 'FI';
    ...
    $pdf['shop_country'] = $c_country_code;
    $pdf['shop_extra_1'] = "Business ID: " . $c_id;

    // Object Sync (Fixes {"default":""} issues)
    ...
    $pdf['shop_address_country']  = array('default' => $c_country_code);
...
function render_purnukka_settings_page() {
    ...
    $countries = ['Finland', 'Sweden', 'Estonia', 'Norway', 'UK', 'USA'];
    ?>
    <div class="wrap">
        <h1 style="color:#c5a059;">Purnukka Stack v0.28</h1>
        <hr>
        <form method="post" action="options.php">
            <?php settings_fields('purnukka-settings-group'); ?>
            <h3>1. Identity & Branding (4-Colors)</h3>
            ...
            <table class="form-table">
                ...
                <tr><th>Address / City / Zip</th><td>
                    ...
                    <input type="text" name="p_legal_city" value="<?php echo esc_attr(get_option('p_legal_city')); ?>" placeholder="City">
                    <input type="text" name="p_legal_postcode" value="<?php echo esc_attr(get_option('p_legal_postcode')); ?>" placeholder="Zip">
                </td></tr>
                <tr><th>Country</th><td>
                    <select name="p_legal_country">
                        <?php foreach($countries as $c): ?>
                            <option value="<?php echo $c; ?>" <?php selected(get_option('p_legal_country'), $c); ?>><?php echo $c; ?></option>
                        <?php endforeach; ?>
                    </select>
                </td></tr>
            </table>
            ...
            <?php submit_button('Save & Sync Global Data'); ?>
        </form>
    </div>
    <?php
}
